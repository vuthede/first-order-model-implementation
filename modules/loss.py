...
class KEquivarianceLoss(nn.Module):
    def __init__(self,loss_weight_dict, device= 'cuda'):
        ...
        self.device = device
    def forward(self, K, x_prime, kp_driving):
        ...
        transformed_kp = K(transformed_frame)  # New predicted keypoint w.r.t the warped image

        ## Loss equivariance_value
        # Randomly transform the driving image with the control points are kp_driving themselves,
        # so that we expect the new kp after transform should stay the same postiion ==> Loss equivariance_value
        loss_equivariance_value = torch.abs(kp_driving['value'] - transform.warp_coordinates(transformed_kp['value'])).mean()
